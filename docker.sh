...
docker stop devops
docker run -d --name ide -p 5901:5900 --net demo-net -e VNC_PASSWORD=nimbusPassw0rd123 dorowu/ubuntu-desktop-lxde-vnc
...
docker run -d --name octane --hostname octane.aos.com --net demo-net --shm-size=2g --ip=172.50.10.6 --restart=always admpresales/octane:12.55.6.67_dis
docker stop octane
docker start octane
docker run -d --hostname autopass.aos.com --ip=172.50.10.10 --name autopass --net demo-net --restart=always admpresales/autopass:9.3_v2
docker run -d --name leanft -p 5900:5900 -e LFT_LIC_SERVER=autopass -e LFT_LIC_ID=23078 -e VERBOSE=true --net demo-net --restart=always functionaltesting/leanft-chrome:14.01
